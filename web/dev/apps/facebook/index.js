...
function ($, common) {
  ...
  function changeShareType() {
    ...
    if (shareType==="wall") {
      enabled = $(".shareTypeWall");
      disabled = $(".shareTypeGroupWall");
    } else {
      enabled = $(".shareTypeGroupWall");
      disabled = $(".shareTypeWall");
    }
    enabled.removeClass('hiddenImportant');
    enabled.addClass('fixedSize');
    disabled.addClass('hiddenImportant');
    if (shareType === "groupWall") {
  ...
  function loadmeup() {
     $(function () {
      common.setupCommonUI("facebook.com");
      changeShareType();
      $("#shareGroupInstead").bind("click", function(evt) {
        var shareControl = $("#selectShareType")[0];
        shareControl.selectedIndex = 1; // index of the groupWall option.
        changeShareType();
      });
    });
  }
